...
//#endregion
//#region src/MangaFox/genreOptions.ts
	const genreOptions = [
		{
			id: "1",
			value: "Action"
...
//#endregion
//#region src/MangaFox/models.ts
	const DOMAIN = "https://fanfox.net";

//#endregion
//#region src/MangaFox/network.ts
	var MangaFoxInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#region src/MangaFox/main.ts
	var MangaFoxExtension = class {
		constructor() {
			...
			_defineProperty(this, "mainInterceptor", new MangaFoxInterceptor("main"));
		...
		async getDiscoverSectionItems(section, metadata) {
			switch (section.id) {
				case "hot-release": return this.getHotRelease(metadata);
				case "new-manga": return this.getNewManga(section, metadata);
				case "latest-updates": return this.getLatestUpdates(section, metadata);
				case "get-genre-section": return this.getGenresSectionItems();
		...
		async getNewManga(section, metadata) {
			...
			const urlBuilder = new URL$1(DOMAIN).addPathComponent("directory");
			if (page > 1) urlBuilder.addPathComponent(`${page}.html?news`);
			else urlBuilder.addPathComponent("?news");
			const request = {
			...
			if (nextPageLink.length > 0) {
				...
				if (nextPageHref) {
					...
					if (pageMatch && pageMatch[1]) nextPage = parseInt(pageMatch[1], 10);
		...
		async getLatestUpdates(section, metadata) {
			...
			const collectedIds = metadata?.collectedIds ?? [];
			const page = metadata?.page ?? 1;
			const urlBuilder = new URL$1(DOMAIN).addPathComponent("releases");
			if (page > 1) urlBuilder.addPathComponent(`${page}.html`);
			const request = {
				url: urlBuilder.toString(),
				method: "GET"
			};
		...
		async getSearchResults(query, metadata) {
			...
			const genreFilter = (await this.getSearchFilters()).find((f) => f.id === "genres");
			const genresFilter = query.metadata?.find((f) => f.id === "genres")?.value;
			if (genresFilter && genreFilter) {
				...
				Object.entries(genresFilter).forEach(([id, inclusion]) => {
					if (inclusion === "included") {
						...
						if (genreOption) urlBuilder.setQueryItem("genres", genreOption.id);
					}
				});
			}
			if (query.title && query.title.trim() !== "") {
				...
				fixedUrl = urlBuilder.toString().replace("title=%22%22", "title=");
			}
			const request = {
				url: fixedUrl,
				method: "GET"
		...
		async getMangaDetails(mangaId) {
			...
			let contentRating = ContentRating$1.EVERYONE;
			const adultGenres = [
				"Adult",
				"Lolicon",
				"Shotacon"
			...
			if (genres.some((genre) => adultGenres.includes(genre.name))) contentRating = ContentRating$1.ADULT;
			else if (genres.some((genre) => matureGenres.includes(genre.name))) contentRating = ContentRating$1.MATURE;
			return {
				mangaId,
				mangaInfo: {
					primaryTitle: title,
		...
		async getChapters(sourceManga) {
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("manga").addPathComponent(sourceManga.mangaId).toString(),
				method: "GET"
			};
			const $ = await this.fetchCheerio(request);
			const chapters = [];
			for (const chapter of $("div#chapterlist ul li").children("a").toArray()) {
				...
				const chapterIdRegex = ($(chapter).attr("href")?.trim())?.match(/\/manga\/[a-zA-Z0-9_]*\/(.*)\//);
				let chapterId = null;
				if (chapterIdRegex && chapterIdRegex[1]) chapterId = chapterIdRegex[1].split("/").pop() ?? null;
		...
		async getChapterDetails(chapter) {
			...
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("manga").addPathComponent(chapter.sourceManga.mangaId).addPathComponent(chapter.chapterId).toString(),
				method: "GET"
			};
			try {
				const $ = await this.fetchCheerio(request);
				...
				if ($("script[src*=chapter_bar]").length > 0) {
					...
					if (script) {
						const urlsMatch = (0, eval)(script).toString().match(/newImgs=\['(.+?)'\]/);
						if (urlsMatch && urlsMatch[1]) {
							const urls = urlsMatch[1].split("','");
							for (const url of urls) pages.push("https:" + url.replace("'", ""));
						}
					}
				} else {
					const script = $("script:contains(function(p,a,c,k,e,d))").html()?.replace("eval", "");
					if (script) {
						...
						if (chapterIdStartLoc > -1) {
							...
							const pagesNumber = Number($(pagesLinksElements[pagesLinksElements.length - 2])?.attr("data-page")) || 0;
							if (pagesNumber > 0) {
								...
								for (let i = 1; i <= pagesNumber; i++) {
									...
									if (responseString) try {
										const deobfuscatedPageScript = (0, eval)(responseString.replace("eval", "")).toString();
										const baseLinkStartPos = deobfuscatedPageScript.indexOf("pix=") + 5;
										const baseLink = deobfuscatedPageScript.substring(baseLinkStartPos, deobfuscatedPageScript.indexOf(";", baseLinkStartPos) - 1);
										...
										const imageLink = deobfuscatedPageScript.substring(imageLinkStartPos, imageLinkEndPos);
				...
				if (pages.length === 0) {
					$("img.img-fluid").each((_, element) => {
						...
						if (src && src.length > 0) pages.push(src);
					});
					$("script:not([src])").each((_, element) => {
						const scriptText = $(element).html() || "";
						if (scriptText.includes("\"images\"") || scriptText.includes("\"pages\"")) try {
							const match = scriptText.match(/(\{.*"images":\s*\[.*\].*\})/);
							if (match && match[1]) {
								const json = JSON.parse(match[1]);
								if (json.images && Array.isArray(json.images)) json.images.forEach((img) => {
									if (typeof img === "string") pages.push(img);
									else if (Array.isArray(img) && img.length > 0) pages.push(img[0]);
								});
							}
						} catch (e) {
				...
				console.error(`Failed to get chapter details: ${error instanceof Error ? error.message : String(error)}`);
		...
		async fetchCheerio(request) {
			...
			return load(Application.arrayBufferToUTF8String(data));
		}
	};
	...
	const parseDate = (date) => {
		...
		if (date.includes("LESS THAN AN HOUR") || date.includes("JUST NOW")) time = new Date(Date.now());
		else if (date.includes("YEAR") || date.includes("YEARS")) time = /* @__PURE__ */ new Date(Date.now() - number * 31556952e3);
		else if (date.includes("MONTH") || date.includes("MONTHS")) time = /* @__PURE__ */ new Date(Date.now() - number * 2592e6);
		else if (date.includes("WEEK") || date.includes("WEEKS")) time = /* @__PURE__ */ new Date(Date.now() - number * 6048e5);
		else if (date.includes("YESTERDAY")) time = /* @__PURE__ */ new Date(Date.now() - 864e5);
		else if (date.includes("DAY") || date.includes("DAYS")) time = /* @__PURE__ */ new Date(Date.now() - number * 864e5);
		else if (date.includes("HOUR") || date.includes("HOURS")) time = /* @__PURE__ */ new Date(Date.now() - number * 36e5);
		else if (date.includes("MINUTE") || date.includes("MINUTES")) time = /* @__PURE__ */ new Date(Date.now() - number * 6e4);
		else if (date.includes("SECOND") || date.includes("SECONDS")) time = /* @__PURE__ */ new Date(Date.now() - number * 1e3);
		else time = new Date(date);
		return time;
	};
	const MangaFox = new MangaFoxExtension();

//#endregion
//#region src/MangaFox/pbconfig.ts
...
//#endregion
//#region src/tests/MangaFox.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaFox tests", logger);
		registerDefaultTests(suite, MangaFox, pbconfig_default);
		await suite.run();
	}
