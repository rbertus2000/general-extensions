...
//#endregion
//#region src/Atsumaru/implementations/shared/models.ts
	const DOMAIN = "https://atsu.moe";

//#endregion
//#region src/Atsumaru/services/network.ts
	var AtsuInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#endregion
//#region src/Atsumaru/implementations/settings-form/forms.ts
	var AtsumaruSettingsForm = class extends Form {
		getSections() {
			return [Section({
				id: "adult-content",
...
//#endregion
//#region src/Atsumaru/implementations/settings-form/main.ts
	function getShowAdult() {
		return Application.getState("atsumaru-show-adult") ?? false;
	}
	function setShowAdult(value) {
		Application.setState(value, "atsumaru-show-adult");
	}
	var SettingsFormProvider = class {
...
//#endregion
//#region src/Atsumaru/implementations/shared/utils.ts
	...
	function buildThumbnailUrl(source) {
		const imagePath = typeof source === "string" ? source : source?.posterSmall ?? source?.posterMedium ?? source?.poster;
		if (!imagePath) return "";
		if (imagePath.startsWith("http")) return imagePath;
		return `${DOMAIN}${imagePath.startsWith("/") ? imagePath : `/static/${imagePath}`}`;
	}
	function getContentRating() {
		return getShowAdult() ? ContentRating$1.ADULT : ContentRating$1.EVERYONE;
	}
	function parseMangaPage(html) {
		const match = html.match(/window\.mangaPage\s*=\s*({[\s\S]*?});/);
		if (!match) throw new Error("Could not find manga data in page");
		return JSON.parse(match[1]).mangaPage;
	}

//#endregion
//#region src/Atsumaru/implementations/chapter-providing/parsers.ts
	function parseChapterList(data, sourceManga, scanlatorMap) {
		return data.chapters.map((ch) => {
			const groupName = ch.scanlationMangaId ? scanlatorMap.get(ch.scanlationMangaId) ?? "No Group" : "No Group";
			const title = ch.title.replace(/^((Chapter|Episode|Ch\.?)\s*[\d.]+|#\s*[\d.]+)\s*(\bS\d+\b)?\s*[-:]?\s*/i, "").trim();
...
//#endregion
//#region src/Atsumaru/implementations/chapter-providing/main.ts
	var ChapterProvider = class {
		async getChapters(sourceManga) {
			const mangaId = sourceManga.mangaId;
			const mangaPage = parseMangaPage(await fetchText({
			...
			return parseChapterList(await fetchJSON({
				url: new URL(DOMAIN).addPathComponent("api").addPathComponent("manga").addPathComponent("allChapters").setQueryItem("mangaId", mangaId).toString(),
				method: "GET"
			}), sourceManga, scanlatorMap);
		}
		async getChapterDetails(chapter) {
			...
			return {
				id: chapterId,
				mangaId,
				pages: (await fetchJSON({
					url: new URL(DOMAIN).addPathComponent("api").addPathComponent("read").addPathComponent("chapter").setQueryItem("mangaId", mangaId).setQueryItem("chapterId", chapterId).toString(),
					method: "GET"
				})).readChapter.pages.sort((a, b) => a.number - b.number).map((page) => page.image.startsWith("http") ? page.image : `${DOMAIN}${page.image}`)
			};
		}
	};

//#endregion
//#region src/Atsumaru/implementations/discover-section/parsers.ts
	function parseDiscoverSections(json) {
		return json.homePage.sections.filter((section) => section.layout === "carousel" && section.key !== "hot-updates").map((section) => ({
			id: section.key,
			title: section.title || "Unknown",
			type: DiscoverSectionType.simpleCarousel
		}));
	}
	function parseDiscoverItems(json, sectionId) {
		const section = json.homePage.sections.find((s) => s.key === sectionId);
...
//#endregion
//#region src/Atsumaru/implementations/discover-section/main.ts
	var DiscoverProvider = class {
		async getDiscoverSections() {
			const showAdult = getShowAdult();
			const url = new URL(DOMAIN).addPathComponent("api").addPathComponent("home").addPathComponent("page");
			if (showAdult) url.setQueryItem("adult", "1");
			return parseDiscoverSections(await fetchJSON({
				url: url.toString(),
				method: "GET"
			}));
		}
		async getDiscoverSectionItems(section, metadata) {
			...
			if (section.id === "top-rated") {
				...
				if (showAdult) url.setQueryItem("adult", "1");
				return {
					items: parseDiscoverItems(await fetchJSON({
						url: url.toString(),
						method: "GET"
					}), section.id),
					metadata: void 0
			...
			if (!endpoint) throw new Error(`Unknown section: ${section.id}`);
			const url = new URL(DOMAIN).addPathComponent("api").addPathComponent("infinite").addPathComponent(endpoint).setQueryItem("page", page.toString()).setQueryItem("types", "Manga,Manwha,Manhua");
			if (showAdult) url.setQueryItem("adult", "1");
...
//#endregion
//#region src/Atsumaru/implementations/manga/parsers.ts
	function parseMangaDetails(html, mangaId) {
		...
		return {
			...
			mangaInfo: {
				...
				author: manga.authors.length > 0 ? manga.authors.map((a) => a.name).join(", ") : void 0,
				status: manga.status,
				contentRating: getContentRating(),
				tagGroups: manga.genres?.length > 0 ? [{
					id: "tags",
					title: "Tags",
...
//#endregion
//#region src/Atsumaru/implementations/manga/main.ts
	var MangaProvider = class {
		async getMangaDetails(mangaId) {
			return parseMangaDetails(await fetchText({
				url: new URL(DOMAIN).addPathComponent("manga").addPathComponent(mangaId).toString(),
				method: "GET"
			}), mangaId);
		}
...
//#region node_modules/@paperback/types/lib/compat/0.8/searchFilters.js
	var SearchFilterForm$1 = class extends AdvancedSearchForm {
...
//#endregion
//#region src/Atsumaru/implementations/search-results/parsers.ts
	function sanitizeMinChapters(value) {
	...
	function extractSearchFilters(query) {
		...
		if (tagsFilter?.value && typeof tagsFilter.value === "object") {
			const tagValue = tagsFilter.value;
		...
		if (typesFilter?.value && typeof typesFilter.value === "object") {
			const typeValue = typesFilter.value;
		...
		if (statusesFilter?.value && typeof statusesFilter.value === "object") {
			const statusValue = statusesFilter.value;
		...
		if (yearsFilter?.value && typeof yearsFilter.value === "object") {
			const yearValue = yearsFilter.value;
		...
		const minChaptersText = typeof minChaptersFilter?.value === "string" ? sanitizeMinChapters(minChaptersFilter.value) : "";
		...
		return {
			...
			selectedStatuses,
			selectedYears,
			minChapters: minChaptersValue !== null && Number.isFinite(minChaptersValue) && minChaptersValue > 0 ? minChaptersValue : null,
			officialTranslation: (query.metadata?.find((f) => f.id === "officialTranslation"))?.value === "true"
		};
	}

//#endregion
//#region src/Atsumaru/implementations/search-results/main.ts
	const PAGE_SIZE = 20;
	var SearchFilterForm = class extends SearchFilterForm$1 {
		getSections() {
			return super.getSections().map((section) => ({
				...
				items: section.items.map((item) => {
					...
					return {
						...item,
						value: sanitizeMinChapters(item.value ?? ""),
						onValueChange: Application.Selector(this, "setMinChapters")
					};
				})
			}));
		}
		async setMinChapters(value) {
	...
	function buildYearOptions() {
		const years = [];
		for (let year = (/* @__PURE__ */ new Date()).getFullYear() + 1; year >= 1970; year--) years.push({
			id: String(year),
			value: String(year)
		});
		return years;
	...
	var SearchProvider = class {
		async getSearchFilters() {
			const filters = await fetchJSON({
				url: new URL(DOMAIN).addPathComponent("api").addPathComponent("explore").addPathComponent("availableFilters").toString(),
				method: "GET"
			});
			const searchFilters = [];
			if (filters.genres && filters.genres.length > 0) searchFilters.push({
				type: "multiselect",
				id: "tags",
				title: "Tags",
			...
			if (filters.types && filters.types.length > 0) searchFilters.push({
				type: "multiselect",
				id: "types",
				title: "Types",
			...
			if (filters.statuses && filters.statuses.length > 0) searchFilters.push({
				type: "multiselect",
				id: "statuses",
				title: "Status",
			...
			searchFilters.push({
				...
				options: [{
					...
					id: "true",
					value: "Only Official Translations"
				}],
				value: ""
			});
		...
		async getSearchResults(query, metadata, sortingOption) {
			...
			const filterBy = [];
			for (const tag of filters.includedTags) filterBy.push(`genreIds:=${escapeFilterValue(tag)}`);
			if (filters.excludedTags.length > 0) filterBy.push(`genreIds:!=[${filters.excludedTags.map((tag) => escapeFilterValue(tag)).join(",")}]`);
			if (filters.selectedTypes.length > 0) filterBy.push(`type:=[${filters.selectedTypes.map((type) => escapeFilterValue(type)).join(",")}]`);
			if (filters.selectedStatuses.length > 0) filterBy.push(`status:=[${filters.selectedStatuses.map((status) => escapeFilterValue(status)).join(",")}]`);
			if (filters.selectedYears.length > 0) filterBy.push(`releaseYear:=[${filters.selectedYears.join(",")}]`);
			if (filters.minChapters !== null) filterBy.push(`chapterCount:>=${filters.minChapters}`);
			if (filters.officialTranslation) filterBy.push("officialTranslation:=true");
			if (!showAdult) filterBy.push("isAdult:=false");
			if (sortBy === "mbRating:desc") filterBy.push("mbRating:>0");
			else if (sortBy === "views:desc") filterBy.push("views:>0");
			const data = await fetchJSON({
				url: new URL(DOMAIN).addPathComponent("collections").addPathComponent("manga").addPathComponent("documents").addPathComponent("search").setQueryItem("q", searchTerm || "*").setQueryItem("query_by", "title,englishTitle,otherNames,authors").setQueryItem("query_by_weights", "4,3,2,1").setQueryItem("num_typos", "4,3,2,1").setQueryItem("include_fields", "id,title,englishTitle,poster,posterSmall,posterMedium,type").setQueryItem("filter_by", filterBy.join(" && ")).setQueryItem("page", String(page)).setQueryItem("per_page", String(PAGE_SIZE)).setQueryItem("sort_by", sortBy).toString(),
				method: "GET"
			});
			const hits = data.hits ?? [];
			return {
				items: hits.map(({ document: manga }) => ({
					mangaId: manga.id,
				...
				metadata: page * PAGE_SIZE < data.found && hits.length > 0 ? { page: page + 1 } : void 0
			};
		}
	};

//#endregion
//#region src/Atsumaru/main.ts
	var AtsumaruExtension = class {
		constructor() {
			...
			_defineProperty(this, "atsuInterceptor", new AtsuInterceptor("atsumaru-interceptor"));
		}
		async initialise() {
		...
		async bypassCloudflareRequest(request) {
			return request;
		}
	};
	...
	const Atsumaru = new AtsumaruExtension();

//#endregion
//#region src/Atsumaru/pbconfig.ts
	var pbconfig_default = {
		...
		badges: [],
		developers: [{ name: "Lucifers Circle" }]
	};
...
//#endregion
//#region src/tests/Atsumaru.ts
	async function runTests(logger) {
		const suite = new TestSuite("Atsumaru tests", logger);
		registerDefaultTests(suite, Atsumaru, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=Atsumaru.js.map