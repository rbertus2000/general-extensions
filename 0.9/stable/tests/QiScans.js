...
//#endregion
//#region src/QiScans/implementations/shared/models.ts
	const DOMAIN = "https://qimanhwa.com";
	const DOMAIN_API = "https://api.qimanhwa.com/api";

//#endregion
//#region src/QiScans/services/network.ts
	var QiScansInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#endregion
//#region src/QiScans/implementations/shared/utils.ts
	...
	function encodeMangaId(value) {
		return encodeURIComponent(value).replace(/[!'()*]/g, (char) => `%${char.charCodeAt(0).toString(16).toUpperCase()}`);
	}
	function decodeMangaId(value) {
		return decodeURIComponent(value);
	}

//#endregion
//#region src/QiScans/implementations/manga/parsers.ts
	function parseMangaDetails(series) {
		...
		return {
			...
			mangaInfo: {
				...
				secondaryTitles: series.alternativeTitles ? series.alternativeTitles.split(/, ?/).map((t) => t.trim()).filter((t) => t.length > 0) : [],
				...
				tagGroups: series.genres && series.genres.length > 0 ? [{
					id: "genres",
					title: "Genres",
...
//#endregion
//#region src/QiScans/implementations/manga/main.ts
	var MangaProvider = class {
		async getMangaDetails(mangaId) {
			const slug = decodeMangaId(mangaId);
			return parseMangaDetails(await fetchJSON({
				url: new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent(slug).toString(),
				method: "GET"
			}));
		}
	};
...
//#endregion
//#region src/QiScans/implementations/chapter-providing/parsers.ts
	function parseChapterList(chapters, sourceManga) {
		if (chapters.length === 0) return [];
		return [...chapters].sort((a, b) => {
			if (a.number !== b.number) return a.number - b.number;
			return new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime();
		}).filter((chapter) => chapter.publishStatus === "PUBLIC" && !chapter.requiresPurchase).map((chapter, index) => ({
	...
	function parseChapterDetails(data, chapter) {
		const pages = [...data.images ?? []].sort((a, b) => a.order - b.order).map((image) => image.url).filter((url) => url.length > 0);
		if (pages.length === 0) throw new Error("No chapter page data could be parsed from QiScans for this chapter.");
		return {
...
//#endregion
//#region src/QiScans/implementations/chapter-providing/main.ts
	var ChapterProvider = class {
		async getChapters(sourceManga) {
			const mangaId = sourceManga.mangaId;
			if (!sourceManga.mangaInfo?.additionalInfo?.slug) sourceManga.mangaInfo = (await new MangaProvider().getMangaDetails(mangaId)).mangaInfo;
			const slug = sourceManga.mangaInfo?.additionalInfo?.slug ?? decodeMangaId(sourceManga.mangaId);
			if (!slug) throw new Error(`Missing slug for ${mangaId}`);
			const chapters = [];
			let page = 1;
			while (true) {
				const data = await fetchJSON({
					url: new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent(slug).addPathComponent("chapters").setQueryItem("page", page.toString()).setQueryItem("perPage", "30").setQueryItem("sort", "desc").toString(),
					method: "GET"
				});
				chapters.push(...data.data ?? []);
				if (!data.next || page >= data.totalPages) break;
				page = data.next;
			}
		...
		async getChapterDetails(chapter) {
			...
			const seriesSlug = sourceManga.mangaInfo?.additionalInfo?.slug ?? decodeMangaId(sourceManga.mangaId);
			return parseChapterDetails(await fetchJSON({
				url: new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent(seriesSlug).addPathComponent("chapters").addPathComponent(chapter.chapterId).toString(),
				method: "GET"
			}), chapter);
		}
	};
...
//#endregion
//#region src/QiScans/implementations/discover-section/parsers.ts
	function formatSeriesSubtitle(type, status) {
		return [type, status].filter((value) => Boolean(value)).map((value) => value.toLowerCase().replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())).join(" • ");
	}
	function isReadableSeries(series) {
		if (!series.title || series.title.trim().length === 0) return false;
		if (series.title.startsWith("http://") || series.title.startsWith("https://")) return false;
		if (!series.slug || series.type === "NOVEL") return false;
		if (series.redirectUrl?.trim()) return false;
		return true;
	}
	function pickReadableChapter(chapters) {
		return chapters?.find((chapter) => chapter.price === 0) ?? chapters?.[0];
	}
	function parseProminentItems(items) {
	...
	function parseSimpleItems(items) {
		return items.filter(isReadableSeries).map((series) => ({
	...
	function parseChapterUpdateItems(items) {
	...
	function parseDiscoverItems(data, sectionId) {
		switch (sectionId) {
			case "featured": return parseProminentItems(data.banners ?? []);
			case "popular": return parseChapterUpdateItems(data.popular ?? []);
			case "pinned": return parseChapterUpdateItems(data.pinned ?? []);
			case "new": return parseChapterUpdateItems(data.newSeries ?? []);
			case "editors-pick": return parseSimpleItems(data.editorsPick ?? []);
			default: return [];
		}
	}

//#endregion
//#region src/QiScans/implementations/discover-section/main.ts
		...
		async getDiscoverSectionItems(section, _metadata) {
			return {
				items: parseDiscoverItems(await fetchJSON({
					url: new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("home").toString(),
					method: "GET"
				}), section.id),
				metadata: void 0
			};
...
//#endregion
//#region src/QiScans/implementations/search-results/parsers.ts
	const SORT_OPTIONS = [
		{
			id: "latest",
	...
	async function fetchGenres() {
		return await fetchJSON({
			url: new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent("genres").toString(),
			method: "GET"
		});
	}
	...
	function formatSearchSubtitle(type, status) {
		return [type, status].filter((value) => Boolean(value)).map((value) => value.toLowerCase().replace(/_/g, " ").replace(/\b\w/g, (char) => char.toUpperCase())).join(" • ");
	}
	function parseSearchResults(data) {
		return (data.data ?? []).filter((series) => {
			if (!series.title || series.title.trim().length === 0) return false;
			if (series.title.startsWith("http://") || series.title.startsWith("https://")) return false;
			if (series.type === "NOVEL") return false;
			if (series.redirectUrl?.trim()) return false;
			return true;
		}).map((series) => {
...
//#endregion
//#region src/QiScans/implementations/search-results/main.ts
	var SearchProvider = class {
		async getSearchResults(query, metadata, sortingOption) {
			...
			let urlBuilder = searchTerm ? new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent("search").setQueryItem("q", searchTerm).setQueryItem("page", page.toString()).setQueryItem("perPage", 20 .toString()) : new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").setQueryItem("page", page.toString()).setQueryItem("perPage", 20 .toString()).setQueryItem("sort", sortingOption?.id ?? "latest");
			if (!searchTerm) {
			...
			if (results.length === 0 && searchTerm.includes("'")) {
				...
				urlBuilder = new URL(DOMAIN_API).addPathComponent("v1").addPathComponent("series").addPathComponent("search").setQueryItem("q", curlySearchTerm).setQueryItem("page", page.toString()).setQueryItem("perPage", 20 .toString());
			...
			const hasNext = (data.data?.length ?? 0) >= 20;
			return {
				items: results,
...
//#endregion
//#region src/QiScans/main.ts
	var QiScansExtension = class {
		constructor() {
			...
			_defineProperty(this, "qiscansInterceptor", new QiScansInterceptor("qiscans-interceptor"));
		}
		async initialise() {
	...
	const QiScans = new QiScansExtension();

//#endregion
//#region src/QiScans/pbconfig.ts
...
//#endregion
//#region src/tests/QiScans.ts
	async function runTests(logger) {
		const suite = new TestSuite("QiScans tests", logger);
		registerDefaultTests(suite, QiScans, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=QiScans.js.map