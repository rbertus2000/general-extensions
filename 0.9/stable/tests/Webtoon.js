...
//#region src/Webtoon/models.ts
	...
	const getLanguagesTitle = (language) => {
		return LanguagesOptions.find((option) => option.id === language)?.title ?? language;
	};
	const getDateDayFormat = () => {
		return new Intl.DateTimeFormat("en-US", { weekday: "long" }).format().toLowerCase();
	};

//#endregion
//#region src/Webtoon/forms.ts
	var WebtoonAdvancedSearchForm = class extends AdvancedSearchForm {
		constructor(searchQuery, genres) {
			super();
	...
	var WebtoonSettings = class {
		...
		get languages() {
			return Application.getState("languages") ?? ["en"];
		}
		set languages(value) {
	...
	var WebtoonSettingForm = class extends Form {
		...
		getSections() {
			return [Section("settings", [SelectRow(LANGUAGES, {
				title: "Languages",
...
//#endregion
//#region src/Webtoon/parsers.ts
	var WebtoonParser = class extends WebtoonSettings {
		parseDetails($, mangaId) {
			const detailElement = $("#wrap > #container > #content > div.cont_box > div.detail_header > div.info");
			const infoElement = $("#_asideDetail");
			const isCanvas = mangaId.startsWith(this.languageFromId(mangaId) + "/canvas");
			const [image, title] = isCanvas ? [this.parseCanvasDetailsThumbnail($), detailElement.find("h3").text().trim()] : [this.parseDetailsThumbnail($), detailElement.find("h1").text().trim()];
			return {
				mangaId,
				mangaInfo: {
					thumbnailUrl: image,
					...
					tagGroups: [{
						...
						tags: detailElement.find(".genre").toArray().map((genre) => ({
							id: $(genre).text().replaceAll(/\s+|\//gm, "_"),
							title: $(genre).text()
						}))
					}],
		...
		parseDetailsThumbnail($) {
			...
			return $("meta[property='og:image']").attr("content") ?? "" ?? thumb;
		}
		parseCanvasDetailsThumbnail($) {
			return $("#content > div.cont_box span.thmb > img").attr("src") ?? "";
		}
		parseChaptersList(dto, sourceManga) {
		...
		parsePopularTitles($) {
			return { items: $("div#content div.webtoon_list_wrap ul.webtoon_list li a").toArray().filter((elem) => $(elem).find("strong.title")).map((elem) => this.parseMangaFromElement($(elem))) };
		}
		parseTodayTitles($, allTitles) {
			const mangas = [];
			const list = $(`div#content div.webtoon_list_wrap ul.webtoon_list li a`);
			for (let i = 0; i <= list.length - 1 && (allTitles || mangas.length < 10); i++) if ($(list[i]).find("strong.title")) mangas.push(this.parseMangaFromElement($(list[i])));
			return { items: mangas };
		}
		parseCanvasRecommendedTitles($) {
			return { items: $("#recommendArea li.rolling-item").toArray().map((elem) => this.parseCanvasFromRecommendedElement($(elem))) };
		}
		parseCanvasFromRecommendedElement(elem) {
			const mangaId = elem.find("a").attr("href")?.replace("https://www.webtoons.com" + "/", "") ?? "";
		...
		parseCanvasPopularTitles($) {
			return { items: $("div.challenge_lst li a").toArray().map((elem) => this.parseCanvasFromElement($(elem))) };
		}
		parseMangaFromElement(elem) {
			const mangaId = elem.attr("href")?.replace("https://www.webtoons.com" + "/", "") ?? "";
			const subtitle = mangaId.startsWith(this.languageFromId(mangaId) + "/canvas") ? "Canvas" + (this.languages.length > 1 ? " - " + this.languageTitleFromId(mangaId) : "") : this.languages.length > 1 ? this.languageTitleFromId(mangaId) : "";
			const contentRating = elem.children().attr("data-title-unsuitable-for-children") == "true" ? ContentRating.MATURE : ContentRating.EVERYONE;
			return {
				mangaId,
		...
		parseSearchResults($) {
			return { items: $("#content > div.webtoon_list_wrap ul li a._card_item").toArray().map((elem) => this.parseMangaFromElement($(elem))) };
		}
		parseGenres($) {
			return $("#content > div#genre_wrap > div.snb_inner > ul li").toArray().map((elem) => this.parseTagFromElement($(elem)));
		}
		parseCanvasGenres($) {
			return $("#content > div#genre_wrap > div.snb_inner > ul li").toArray().map((elem) => this.parseCanvasTagFromElement($(elem)));
		}
		...
		parseCanvasTagFromElement(elem) {
			return {
				id: "CANVAS%%" + elem.find("a").text().trim().replaceAll(/\s+|\//gm, "_"),
				title: "Canvas - " + elem.find("a").text().trim()
			};
		}
		parseTagResults($) {
			return { items: $("#content > div.webtoon_list_wrap ul li a").toArray().map((elem) => this.parseMangaFromElement($(elem))) };
		}
		languageFromId(id) {
			return id.split("/")[0];
		}
...
//#endregion
//#region src/Webtoon/network.ts
	var WebtoonInfra = class extends WebtoonParser {
		constructor() {
			super();
			_defineProperty(this, "cheerio", browser_exports);
			_defineProperty(this, "websiteRateLimiter", new WebtoonWebSiteRateLimiter());
			_defineProperty(this, "mobileApiRateLimiter", new WebtoonMobileApiRateLimiter());
			_defineProperty(this, "cookies", void 0);
			this.cookies = { ageGatePass: "true" };
		}
		async initialise() {
		...
		registerInterceptors() {
			...
			Application.registerInterceptor("requestInterceptor", Application.Selector(this, "interceptRequest"), Application.Selector(this, "interceptResponse"));
		}
		async interceptRequest(request) {
			request.headers = {
				...request.headers,
				referer: request.headers?.referer ?? `${"https://www.webtoons.com"}/`,
		...
		async ExecRequest(infos, parseMethods) {
			if (infos.params) infos.url += `?${Object.entries(infos.params).map(([key, value]) => `${key}=${value}`).join("&")}`;
			const request = {
				...infos,
				method: "GET"
			};
		...
		async ExecApiRequest(infos, parseMethods) {
			...
			const data = (await Application.scheduleRequest(request))[1];
			const rootDto = JSON.parse(Application.arrayBufferToUTF8String(data));
		...
		async ExecPagedResultsRequest(infos, metadata, parseMethods) {
			infos.params ?? (infos.params = {});
			const page = infos.params.page = metadata.page + 1;
			if (metadata?.maxPages && page > metadata.maxPages) return {
				items: [],
				metadata: void 0
			};
			...
			return {
				items: results.items,
				metadata: results.items.length != 0 ? {
					...metadata,
					page
				} : void 0
			};
	...
	var WebtoonWebSiteRateLimiter = class extends BasicRateLimiter {
		constructor() {
		...
		async interceptRequest(request) {
			if (request.url.startsWith("https://m.webtoons.com")) return request;
			return super.interceptRequest(request);
		}
	};
	var WebtoonMobileApiRateLimiter = class extends BasicRateLimiter {
		constructor() {
...
//#endregion
//#region src/Webtoon/main.ts
	var WebtoonExtention = class extends WebtoonInfra {
		getMangaDetails(mangaId) {
			return this.ExecRequest({ url: `${BASE_URL}/${mangaId}` }, ($) => this.parseDetails($, mangaId));
		}
		getChapters(sourceManga) {
			const titleUrl = new URL$1(`${BASE_URL}/${sourceManga.mangaId}`);
			const titleId = titleUrl.queryItems["title_no"].toString();
			const segment = titleUrl.path.includes("/canvas/") ? "canvas" : "webtoon";
			return this.ExecApiRequest({
				url: `${MOBILE_URL}/api/v1/${segment}/${titleId}/episodes`,
				params: { pageSize: 99999 },
				headers: { referer: MOBILE_URL }
			}, (dto) => this.parseChaptersList(dto, sourceManga));
		}
		getChapterDetails(chapter) {
			return this.ExecRequest({ url: `${BASE_URL}/${chapter.chapterId}` }, ($) => this.parseChapterDetails($, chapter));
		}
		getPopularTitles(language, metadata) {
			return this.ExecPagedResultsRequest({ url: `${BASE_URL}/${language}/ranking/popular` }, {
				page: metadata?.page ?? 0,
				maxPages: 1
			}, ($) => this.parseTodayTitles($, true));
		}
		getTodayTitles(language, metadata) {
			return this.ExecPagedResultsRequest({ url: `${BASE_URL}/${language}/originals/${getDateDayFormat()}` }, {
				page: metadata?.page ?? 0,
				maxPages: 1
			}, ($) => this.parseTodayTitles($, true));
		}
		getTrendingTitles(language, metadata) {
			return this.ExecPagedResultsRequest({ url: `${BASE_URL}/${language}/ranking/trending` }, {
				page: metadata?.page ?? 0,
				maxPages: 1
			}, ($) => this.parseTodayTitles($, true));
		}
		getCompletedTitles(language, metadata) {
			return this.ExecPagedResultsRequest({ url: `${BASE_URL}/${language}/originals/complete` }, {
				page: metadata?.page ?? 0,
				maxPages: 1
			}, ($) => this.parseTodayTitles($, true));
		}
		getCanvasRecommendedTitles(language) {
			return this.ExecRequest({ url: `${BASE_URL}/${language}/canvas` }, ($) => this.parseCanvasRecommendedTitles($));
		}
		getCanvasPopularTitles(language, metadata, genre, sortOrder) {
			...
			}, { page: metadata?.page ?? 0 }, ($) => this.parseCanvasPopularTitles($));
		}
		getTitlesByGenre(language, genre, sortOrder) {
		...
		getTitlesByKeyword(language, keyword, metadata) {
			return this.ExecPagedResultsRequest({
				...
				params: { keyword }
			}, { page: metadata?.page ?? 0 }, ($) => this.parseSearchResults($));
		}
		async getSearchResults(query, metadata, sortingOption) {
			...
			const genres = searchMetadata.genres.length !== 0 ? searchMetadata.genres.join("%%") : "ALL";
			(searchMetadata.languages.length !== 0 ? searchMetadata.languages : this.languages).forEach((lang) => {
				result.push(genres !== "ALL" ? genres.startsWith("CANVAS%%") ? this.getCanvasPopularTitles(lang, metadata, genres.split("%%")[1], sortingOption) : this.getTitlesByGenre(lang, genres, sortingOption) : query.title ? this.getTitlesByKeyword(lang, query.title, metadata) : Promise.resolve({ items: [] }));
			});
			return Promise.all(result).then((res) => {
				return {
					items: res.flatMap((r) => r.items),
					metadata: res.filter((r) => r.metadata != void 0)[0]?.metadata ?? void 0
				};
			});
		}
		...
		async getSearchGenres() {
			return [
				...
				...this.canvasWanted ? await this.ExecRequest({ url: `${BASE_URL}/en/canvas` }, ($) => this.parseCanvasGenres($)) : []
			];
		}
		...
		async getAdvancedSearchForm(searchQuery) {
			return new WebtoonAdvancedSearchForm(searchQuery, await this.getSearchGenres());
		}
	};
	const Webtoon = new WebtoonExtention();

//#endregion
//#region src/Webtoon/pbconfig.ts
...
//#endregion
//#region src/tests/Webtoon.ts
	async function runTests(logger) {
		const suite = new TestSuite("Webtoon tests", logger);
		registerDefaultTests(suite, Webtoon, pbconfig_default);
		await suite.run();
	}
