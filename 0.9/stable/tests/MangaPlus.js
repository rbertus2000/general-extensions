...
//#endregion
//#region src/MangaPlus/MangaPlusHelper.ts
	function langPopup(errorResult, lang) {
		return errorResult?.popups?.find((popup) => (popup.language ?? "ENGLISH") === lang) || null;
	}
	var Title = class {
		constructor(titleId, name, portraitImageUrl, landscapeImageUrl, author) {
			_defineProperty(this, "titleId", void 0);
			_defineProperty(this, "name", void 0);
			_defineProperty(this, "author", void 0);
			_defineProperty(this, "portraitImageUrl", void 0);
			_defineProperty(this, "landscapeImageUrl", void 0);
			_defineProperty(this, "viewCount", 0);
			_defineProperty(this, "language", "ENGLISH");
	...
	var TitleDetailView = class TitleDetailView {
		constructor() {
			_defineProperty(this, "title", void 0);
			_defineProperty(this, "titleImageUrl", void 0);
			_defineProperty(this, "overview", void 0);
			_defineProperty(this, "backgroundImageUrl", void 0);
			_defineProperty(this, "nextTimeStamp", 0);
			_defineProperty(this, "viewingPeriodDescription", "");
			_defineProperty(this, "nonAppearanceInfo", "");
			_defineProperty(this, "chapterListGroup", []);
			_defineProperty(this, "firstChapterList", []);
			_defineProperty(this, "lastChapterList", []);
			_defineProperty(this, "isSimulReleased", false);
			_defineProperty(this, "chaptersDescending", true);
		}
		get isWebtoon() {
		...
		static fromJson(str) {
			...
			if (json.title === void 0) throw Error("Cannot find title");
	...
	_defineProperty(TitleDetailView, "COMPLETED_REGEX", /completado|complete|completo/);
	_defineProperty(TitleDetailView, "HIATUS_REGEX", /on a hiatus/i);
	_defineProperty(TitleDetailView, "REEDITION_REGEX", /revival|remasterizada/);
	var Chapter = class {
		constructor(titleId, chapterId, name, startTimeStamp, endTimeStamp) {
			...
			_defineProperty(this, "endTimeStamp", void 0);
			_defineProperty(this, "isVerticalOnly", false);
			this.titleId = titleId;
		...
		toSChapter(sourceManga) {
			...
			return {
				...
				publishDate: /* @__PURE__ */ new Date(this.startTimeStamp * 1e3)
			};
		}
	};

//#endregion
//#region src/MangaPlus/MangaPlusSettings.ts
	const getLanguages = () => {
		return Application.getState("languages") ?? ["ENGLISH"];
	};
	const getSplitImages = () => {
		return Application.getState("split_images") ?? "yes";
	};
	const getResolution = () => {
		return Application.getState("image_resolution") ?? "high";
	};
	var MangaPlusSettingForm = class extends Form {
		getSections() {
			return [Section("content_settings", [
				SelectRow("languages", {
					title: "Languages",
					value: getLanguages(),
					minItemCount: 1,
			...
			]), Section("reset_settings", [ButtonRow("reset", {
		...
		async resetSettings() {
			Application.setState(["ENGLISH"], "languages");
...
//#region src/MangaPlus/main.ts
	...
	var MangaPlusExtension = class {
		...
		async getMangaDetails(mangaId) {
			...
			return TitleDetailView.fromJson(Application.arrayBufferToUTF8String(response)).toSourceManga();
		}
		async getThumbnailUrl(mangaId) {
			...
			const response = (await Application.scheduleRequest(request))[1];
			return TitleDetailView.fromJson(Application.arrayBufferToUTF8String(response)).title?.portraitImageUrl ?? "";
		}
		...
		async getChapterDetails(chapter) {
			...
			if (result.success === void 0) throw new Error(langPopup(result.error, "ENGLISH")?.body ?? "Unknown error");
		...
		async getFeaturedTitles() {
			...
			const results = result.success?.featuredTitlesViewV2?.contents?.find((x) => x.titleList && x.titleList.listName == "WEEKLY SHONEN JUMP")?.titleList.featuredTitles.filter((title) => languages.includes(title.language ?? "ENGLISH"));
			const titles = [];
			const collectedIds = [];
		...
		async getPopularTitles() {
			...
			if (result.success === void 0) throw new Error(langPopup(result.error, "ENGLISH")?.body ?? "Unknown error");
			const languages = getLanguages();
			const results = result.success?.titleRankingView?.titles.filter((title) => languages.includes(title.language ?? "ENGLISH"));
			const titles = [];
			const collectedIds = [];
			for (const item of results ?? []) {
				const mangaId = item.titleId.toString();
		...
		async getLatestUpdates() {
			...
			const response = (await Application.scheduleRequest(request))[1];
			const result = JSON.parse(Application.arrayBufferToUTF8String(response));
			if (result.success === void 0) throw new Error(langPopup(result.error, langCode)?.body ?? "Unknown error");
			const languages = getLanguages();
			const results = result.success.webHomeViewV4?.groups.flatMap((ex) => ex.titleGroups).flatMap((ex) => ex.titles).map((title) => title.title).filter((title) => languages.includes(title.language ?? "ENGLISH"));
			const titles = [];
			const collectedIds = [];
			for (const item of results ?? []) {
				const mangaId = item.titleId.toString();
		...
		async getSearchResults(query, metadata) {
			...
			const languages = getLanguages();
			const results = result.success?.allTitlesViewV2?.AllTitlesGroup.flatMap((group) => group.titles).filter((title) => languages.includes(title.language ?? "ENGLISH")).filter((title) => title.author?.toLowerCase().includes(ltitle) || title.name.toLowerCase().includes(ltitle));
			const titles = [];
			const collectedIds = [];
			for (const item of results ?? []) {
				const mangaId = item.titleId.toString();
		...
		registerInterceptors() {
			...
			Application.registerInterceptor("mangaPlusInterceptor", Application.Selector(this, "interceptRequest"), Application.Selector(this, "interceptResponse"));
		}
		async interceptRequest(request) {
			request.headers = {
				...request.headers,
		...
		async interceptResponse(request, response, data) {
			if (!request.url.includes("encryptionKey") && response.headers["Content-Type"] !== "image/jpeg") return data;
			if (request.url.includes("title_thumbnail_portrait_list")) return data;
			const encryptionKey = request.url.substring(request.url.lastIndexOf("#") + 1) ?? "";
			return this.decodeXoRCipher(new Uint8Array(data), encryptionKey).buffer;
		}
		getDiscoverSections() {
	...
	const MangaPlus = new MangaPlusExtension();

//#endregion
//#region src/MangaPlus/pbconfig.ts
...
//#endregion
//#region src/tests/MangaPlus.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaPlus tests", logger);
		registerDefaultTests(suite, MangaPlus, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=MangaPlus.js.map