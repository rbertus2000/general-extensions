...
/**
	...
	*/
	async function getVmToken(pathOnly, cookieInterceptor) {
...
//#region src/Comix/network.ts
	...
	var ComixApi = class {
		...
		async fetchSignedApi(path, query, cookieInterceptor) {
		...
		async getJsonChapterApi(chapter, page, cookieStorageInterceptor) {
			return this.fetchSignedApi(`/manga/${chapter}/chapters`, {
				page: page.toString(),
				limit: "100",
				"order[number]": "desc"
			}, cookieStorageInterceptor);
		}
		async getJsonChapterApiBatch(chapter, fromPage, toPage, cookieStorageInterceptor) {
			const pathOnly = `/manga/${chapter}/chapters`;
			const pages = [];
			for (let page = fromPage; page <= toPage; page++) pages.push(page);
			return Promise.all(pages.map((page) => this.fetchSignedApi(pathOnly, {
				page: page.toString(),
				limit: "100",
				"order[number]": "desc"
			}, cookieStorageInterceptor)));
		...
		async getJsonChapPagesApi(chapterId, cookieStorageInterceptor) {
			return this.fetchSignedApi(`/chapters/${chapterId}`, void 0, cookieStorageInterceptor);
		}
...
//#region src/Comix/main.ts
	var ComixExtension = class {
		...
		async getChapters(sourceManga) {
			...
			if (totalPages === 1) return this.parser.parseChapters(sourceManga, firstPage.result.items);
			const remainingPages = await this.api.getJsonChapterApiBatch(sourceManga.mangaId, 2, totalPages, this.cookieStorageInterceptor);
		...
		async getChapterDetails(chapter) {
			const pages = await this.api.getJsonChapPagesApi(chapter.chapterId, this.cookieStorageInterceptor);
