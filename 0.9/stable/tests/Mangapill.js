...
//#endregion
//#region src/Mangapill/models.ts
	const DOMAIN = "https://mangapill.com";
...
//#endregion
//#region src/Mangapill/pbconfig.ts
...
//#endregion
//#region src/Mangapill/parsers.ts
	const parseMangaDetails = async ($, mangaId) => {
		...
		return {
			...
			mangaInfo: {
				...
				shareUrl: new URL$1(DOMAIN).addPathComponent("series").addPathComponent(mangaId).toString()
			}
		};
	};
	const parseChapters = ($, sourceManga) => {
		...
		for (const chapterObj of arrChapters) {
			const chapterId = $(chapterObj).attr("href")?.split("/")[2] ?? "";
			const title = $(chapterObj).text().trim();
			let chapNum = 0;
			let match = $(chapterObj).text().trim().match(/Chapter (\d+)/);
			if (match && match[1]) chapNum = parseFloat(match[1]);
			let volume = 1;
			match = $(chapterObj).text().trim().match(/Group (\d+)/);
			if (match && match[1]) {
				hasVolume = true;
	...
	const parseTrendingSection = async ($) => {
		...
		for (const trendingObj of $("> div", $(".grid-cols-2").last()).toArray()) {
			...
			const imageUrl = $("a img", trendingObj).attr("src") ?? $("a img", trendingObj).attr("data-src") ?? "";
...
//#endregion
//#region src/Mangapill/helpers.ts
	function formatTagId(tagId) {
		return tagId.replaceAll(" ", "_");
	}
	async function getSearchTags() {
		const request = {
			url: new URL$1(DOMAIN).addPathComponent("search").toString(),
			method: "GET"
		};
		const [_, buffer] = await Application.scheduleRequest(request);
		return await parseTags(load(Application.arrayBufferToUTF8String(buffer)));
	}
	function getGenresFromTags(tags) {
		return tags[0];
	}
	function getTypesFromTags(tags) {
		return tags[1];
	}
	function getStatusesFromTags(tags) {
		return tags[2];
	}

//#endregion
//#region src/Mangapill/forms.ts
	var MangapillAdvancedSearchForm = class extends AdvancedSearchForm {
		constructor(searchQuery, tags) {
			...
			else this.searchMetadata = { genres: [] };
			this.tags = tags;
		}
		...
		getStatusesFilter(tags) {
			const tag = getStatusesFromTags(tags);
			return [SelectRow("statuses", {
				title: tag.title,
				...
				onValueChange: Application.Selector(this, "handleStatusesChange")
			})];
		}
		async handleStatusesChange(value) {
			this.searchMetadata.statuses = value;
		}
		async formDidSubmit() {
			if (this.searchMetadata.statuses && this.searchMetadata.statuses.length > 1) throw new Error("Only one status can be selected");
			if (this.searchMetadata.types && this.searchMetadata.types.length > 1) throw new Error("Only one type can be selected");
		}
	};

//#endregion
//#region src/Mangapill/interceptors.ts
	var MangapillInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			request.headers = {
				...request.headers,
				referer: `${DOMAIN}/`
...
//#endregion
//#region src/Mangapill/requests.ts
	async function fetchHomepage() {
		const request = {
			url: new URL$1(DOMAIN).toString(),
			method: "GET"
		};
	...
	async function fetchChapterDetailsPage(chapterId) {
		const request = {
			url: new URL$1(DOMAIN).addPathComponent("chapters").addPathComponent(chapterId).toString(),
			method: "GET"
		};
		return await Application.scheduleRequest(request);
	}
	async function fetchSearchPage(paths, queries) {
		const urlBuilder = new URL$1(DOMAIN).addPathComponent("search");
...
//#region src/Mangapill/main.ts
	var MangapillExtension = class {
		constructor() {
			...
			_defineProperty(this, "requestManager", new MangapillInterceptor("main"));
		}
		async initialise() {
		...
		async getDiscoverSectionItems(section, metadata) {
			...
			switch (section.id) {
				case "trending": {
					...
					items = await parseTrendingSection(load(Application.arrayBufferToUTF8String(buffer)));
					break;
				}
				case "recent": {
					const [_, buffer] = await fetchHomepage();
					items = await parseRecentSection(load(Application.arrayBufferToUTF8String(buffer)));
					break;
				}
				case "genre": items = getGenresFromTags(await getSearchTags()).tags.map((genre) => ({
		...
		async getSearchResults(query, metadata) {
			...
			return {
				items: await parseSearch(load(Application.arrayBufferToUTF8String(response[1]))),
				metadata
			};
		}
		async getAdvancedSearchForm(searchQuery) {
			return new MangapillAdvancedSearchForm(searchQuery, await this.getSearchTags());
		}
	};
	const Mangapill = new MangapillExtension();
...
//#endregion
//#region src/tests/Mangapill.ts
	async function runTests(logger) {
		const suite = new TestSuite("Mangapill tests", logger);
		registerDefaultTests(suite, Mangapill, pbconfig_default);
		await suite.run();
	}
