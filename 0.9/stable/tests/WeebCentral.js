...
//#region node_modules/htmlparser2/node_modules/entities/dist/esm/decode.js
	...
	const TO_LOWER_BIT$2 = 32;
	function isNumber$2(code) {
		return code >= CharCodes$3.ZERO && code <= CharCodes$3.NINE;
	}
	function isHexadecimalCharacter$2(code) {
		return code >= CharCodes$3.UPPER_A && code <= CharCodes$3.UPPER_F || code >= CharCodes$3.LOWER_A && code <= CharCodes$3.LOWER_F;
	}
	function isAsciiAlphaNumeric$3(code) {
		return code >= CharCodes$3.UPPER_A && code <= CharCodes$3.UPPER_Z || code >= CharCodes$3.LOWER_A && code <= CharCodes$3.LOWER_Z || isNumber$2(code);
	}
	/**
	...
	var EntityDecoder$2 = class {
		...
		write(input, offset) {
			switch (this.state) {
				case EntityDecoderState$2.EntityStart:
					if (input.charCodeAt(offset) === CharCodes$3.NUM) {
				...
				case EntityDecoderState$2.NumericStart: return this.stateNumericStart(input, offset);
				case EntityDecoderState$2.NumericDecimal: return this.stateNumericDecimal(input, offset);
				case EntityDecoderState$2.NumericHex: return this.stateNumericHex(input, offset);
				case EntityDecoderState$2.NamedEntity: return this.stateNamedEntity(input, offset);
			}
		}
		/**
		...
		*/
		stateNumericStart(input, offset) {
			...
			if ((input.charCodeAt(offset) | TO_LOWER_BIT$2) === CharCodes$3.LOWER_X) {
		...
		stateNumericHex(input, offset) {
			while (offset < input.length) {
				...
				if (isNumber$2(char) || isHexadecimalCharacter$2(char)) {
					const digit = char <= CharCodes$3.NINE ? char - CharCodes$3.ZERO : (char | TO_LOWER_BIT$2) - CharCodes$3.LOWER_A + 10;
		...
		*/
		stateNumericDecimal(input, offset) {
			while (offset < input.length) {
				const char = input.charCodeAt(offset);
				if (isNumber$2(char)) {
					this.result = this.result * 10 + (char - CharCodes$3.ZERO);
					this.consumed++;
					offset++;
		...
		*/
		emitNumericEntity(lastCp, expectedLength) {
			...
			if (lastCp === CharCodes$3.SEMI) this.consumed += 1;
			else if (this.decodeMode === DecodingMode$2.Strict) return 0;
			this.emitCodePoint(replaceCodePoint$2(this.result), this.consumed);
			if (this.errors) {
				if (lastCp !== CharCodes$3.SEMI) this.errors.missingSemicolonAfterCharacterReference();
		...
		*/
		stateNamedEntity(input, offset) {
			...
			while (offset < input.length) {
				if (valueLength === 0 && (current & BinTrieFlags$2.FLAG13) !== 0) {
					const runLength = (current & BinTrieFlags$2.BRANCH_LENGTH) >> 7;
					if (this.runConsumed === 0) {
						const firstChar = current & BinTrieFlags$2.JUMP_TABLE;
				...
				const char = input.charCodeAt(offset);
				if (char === CharCodes$3.SEMI && valueLength !== 0 && (current & BinTrieFlags$2.FLAG13) !== 0) return this.emitNamedEntityData(this.treeIndex, valueLength, this.consumed + this.excess);
				this.treeIndex = determineBranch$2(decodeTree, current, this.treeIndex + Math.max(1, valueLength), char);
				if (this.treeIndex < 0) return this.result === 0 || this.decodeMode === DecodingMode$2.Attribute && (valueLength === 0 || isEntityInAttributeInvalidEnd$2(char)) ? 0 : this.emitNotTerminatedNamedEntity();
				current = decodeTree[this.treeIndex];
				valueLength = (current & BinTrieFlags$2.VALUE_LENGTH) >> 14;
				if (valueLength !== 0) {
					if (char === CharCodes$3.SEMI) return this.emitNamedEntityData(this.treeIndex, valueLength, this.consumed + this.excess);
					if (this.decodeMode !== DecodingMode$2.Strict && (current & BinTrieFlags$2.FLAG13) === 0) {
		...
		*/
		emitNamedEntityData(result, valueLength, consumed) {
			const { decodeTree } = this;
			this.emitCodePoint(valueLength === 1 ? decodeTree[result] & ~(BinTrieFlags$2.VALUE_LENGTH | BinTrieFlags$2.FLAG13) : decodeTree[result + 1], consumed);
		...
		*/
			...
			var _a;
			switch (this.state) {
				case EntityDecoderState$2.NamedEntity: return this.result !== 0 && (this.decodeMode !== DecodingMode$2.Attribute || this.result === this.treeIndex) ? this.emitNotTerminatedNamedEntity() : 0;
				case EntityDecoderState$2.NumericDecimal: return this.emitNumericEntity(0, 2);
				case EntityDecoderState$2.NumericHex: return this.emitNumericEntity(0, 3);
				case EntityDecoderState$2.NumericStart:
					(_a = this.errors) === null || _a === void 0 || _a.absenceOfDigitsInNumericCharacterReference(this.consumed);
					return 0;
				case EntityDecoderState$2.EntityStart: return 0;
			}
		}
	};
	/**
	...
	*/
	function determineBranch$2(decodeTree, current, nodeIndex, char) {
...
//#region node_modules/htmlparser2/dist/esm/Tokenizer.js
	...
	function isEndOfTagSection(c) {
		return c === CharCodes$2.Slash || c === CharCodes$2.Gt || isWhitespace$2(c);
	}
	...
	*/
	...
	var Tokenizer$1 = class {
		constructor({ xmlMode = false, decodeEntities = true }, cbs) {
			...
			this.entityDecoder = new EntityDecoder$2(xmlMode ? xmlDecodeTree : htmlDecodeTree$2, (cp, consumed) => this.emitCodePoint(cp, consumed));
		}
		reset() {
			this.state = State$1.Text;
			this.buffer = "";
		...
		stateText(c) {
			...
			} else if (this.decodeEntities && c === CharCodes$2.Amp) this.startEntity();
		}
		...
		stateInSpecialTag(c) {
			if (this.sequenceIndex === this.currentSequence.length) {
				if (c === CharCodes$2.Gt || isWhitespace$2(c)) {
			...
			if ((c | 32) === this.currentSequence[this.sequenceIndex]) this.sequenceIndex += 1;
				...
				if (this.currentSequence === Sequences.TitleEnd) {
					if (this.decodeEntities && c === CharCodes$2.Amp) this.startEntity();
				} else if (this.fastForwardTo(CharCodes$2.Lt)) this.sequenceIndex = 1;
			} else this.sequenceIndex = Number(c === CharCodes$2.Lt);
		}
		...
		*/
		...
		stateBeforeTagName(c) {
			...
			} else if (c === CharCodes$2.Slash) this.state = State$1.BeforeClosingTagName;
			else {
				this.state = State$1.Text;
				this.stateText(c);
			}
		...
		stateBeforeClosingTagName(c) {
			if (isWhitespace$2(c)) {} else if (c === CharCodes$2.Gt) this.state = State$1.Text;
			else {
		...
		stateBeforeAttributeName(c) {
			if (c === CharCodes$2.Gt) {
				...
				this.sectionStart = this.index + 1;
			} else if (c === CharCodes$2.Slash) this.state = State$1.InSelfClosingTag;
		...
		stateAfterAttributeName(c) {
			if (c === CharCodes$2.Eq) this.state = State$1.BeforeAttributeValue;
		...
		stateInAttributeValueNoQuotes(c) {
			if (isWhitespace$2(c) || c === CharCodes$2.Gt) {
				this.cbs.onattribdata(this.sectionStart, this.index);
				this.sectionStart = -1;
		...
		stateBeforeDeclaration(c) {
			if (c === CharCodes$2.OpeningSquareBracket) {
				...
				this.sequenceIndex = 0;
			} else this.state = c === CharCodes$2.Dash ? State$1.BeforeComment : State$1.InDeclaration;
		}
		...
		startEntity() {
			...
			this.entityDecoder.startEntity(this.xmlMode ? DecodingMode$2.Strict : this.baseState === State$1.Text || this.baseState === State$1.InSpecialTag ? DecodingMode$2.Legacy : DecodingMode$2.Attribute);
		}
		stateInEntity() {
			...
			if (length >= 0) {
				...
				if (indexInBuffer < this.buffer.length && this.buffer.charCodeAt(indexInBuffer) === CharCodes$2.Amp) {
					this.state = this.baseState;
					this.index -= 1;
					return;
				}
...
//#region node_modules/parse5/node_modules/entities/dist/esm/decode.js
	...
	*/
	function isEntityInAttributeInvalidEnd$1(code) {
		return code === CharCodes$1.EQUALS || isAsciiAlphaNumeric$2(code);
	}
	...
	var Tokenizer = class {
		constructor(options, handler) {
			...
			*/
			...
			this.entityDecoder = new EntityDecoder$1(htmlDecodeTree$1, (cp, consumed) => {
		...
		_stateCharacterReference() {
			...
			if (length === 0) {
				...
				this.state = !this._isCharacterReferenceInAttribute() && isAsciiAlphaNumeric$1(this.preprocessor.peek(1)) ? State.AMBIGUOUS_AMPERSAND : this.returnState;
...
//#endregion
//#region src/WeebCentral/network.ts
	var WeebCentralInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
	...
	async function fetchHomepage() {
		return await Application.scheduleRequest({
			url: new URL$1(WC_DOMAIN).toString(),
			method: "GET"
		});
	}
	async function fetchMangaDetailsPage(mangaId) {
		const request = {
			url: new URL$1(WC_DOMAIN).addPathComponent("series").addPathComponent(mangaId).toString(),
			method: "GET"
		};
		return await Application.scheduleRequest(request);
	}
	async function fetchChaptersPage(mangaId) {
		const request = {
			url: new URL$1(WC_DOMAIN).addPathComponent("series").addPathComponent(mangaId).addPathComponent("full-chapter-list").toString(),
			method: "GET"
		};
		return await Application.scheduleRequest(request);
	}
	async function fetchChapterDetailsPage(chapterId) {
		const request = {
			url: new URL$1(WC_DOMAIN).addPathComponent("chapters").addPathComponent(chapterId).addPathComponent("images").setQueryItem("reading_style", "long_strip").toString(),
			method: "GET"
		};
		return await Application.scheduleRequest(request);
	}
	async function fetchSearchPage(paths, queries) {
		const urlBuilder = new URL$1(WC_DOMAIN).addPathComponent("search");
		for (const path of paths) urlBuilder.addPathComponent(path);
		for (const query of queries) urlBuilder.setQueryItem(query.key, query.value);
		const request = {
	...
	async function fetchRecentViewMorePage(page) {
		const request = {
			url: new URL$1(WC_DOMAIN).addPathComponent("latest-updates").addPathComponent(page.toString()).toString(),
			method: "GET"
		};
		return await Application.scheduleRequest(request);
	}
...
//#endregion
//#region src/WeebCentral/parsers.ts
	...
	const parseChapters = ($, sourceManga) => {
		...
		for (const chapterObj of arrChapters) {
			...
			sortingIndex--;
			if (!(chapterType in chapterTypeToIdMap)) chapterTypeToIdMap[chapterType] = currChapterTypeId--;
			const hasOfficialTranslation = $("svg", chapterObj).attr("stroke") === officialTranslationSvgStroke;
			let version = void 0;
			if (hasOfficialTranslation) version = "Official";
			chapters.push({
				chapterId,
				title,
				chapNum,
		...
		if (totalTypes > 1) for (const chapter of chapters) chapter.volume = totalTypes + chapterTypeToIdMap[chapter.chapterType];
		return chapters;
	};
	...
	const parseRecommendedSection = async ($) => {
		...
		for (const recommendationObj of $(".glide__slide:not(.glide__slide--clone)").toArray()) {
			...
			const imageUrl = $("source", recommendationObj).first().attr("srcset") ?? $("img", recommendationObj).attr("src") ?? "";
	...
	const parseRecentSection = async ($) => {
		const recentSectionArray = [];
		for (const recentObj of $("article", "section.bg-base-200.rounded-sm").toArray()) recentSectionArray.push(parseRecentObject(recentObj, $));
		return recentSectionArray;
	};
	const parseRecentSectionViewMore = async ($) => {
		const recentSectionArray = [];
		for (const recentObj of $("article").toArray()) recentSectionArray.push(parseRecentObject(recentObj, $));
		return recentSectionArray;
	};
	const parseRecentObject = (recentObj, $) => {
		const id = $("a.aspect-square", recentObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
		const chapterId = $("a.min-w-0", recentObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
		const title = $("div.font-semibold", recentObj).text().trim() ?? "";
		const imageUrl = $("a img", recentObj).attr("src") ?? $("a img", recentObj).attr("data-src") ?? "";
	...
	const parseTags = async ($) => {
		const tagSections = [];
		const genreTagSection = parseIntoTagSection($, "included_tag", "Genres", "div:contains(\"Tags\").collapse-title");
		tagSections.push(genreTagSection);
		const seriesStatusTagSection = parseIntoTagSection($, "included_status", "Series Status", "div:contains(\"Series Status\").collapse-title");
		tagSections.push(seriesStatusTagSection);
		const seriesTypeTagSection = parseIntoTagSection($, "included_type", "Series Type", "div:contains(\"Series Type\").collapse-title");
		tagSections.push(seriesTypeTagSection);
		const orderTagSection = parseIntoTagSection($, "order", "Order", "div:contains(\"Order\").collapse-title");
		tagSections.push(orderTagSection);
		return tagSections;
	};
	const parseIntoTagSection = ($, id, title, selector) => {
		const tagSection = {
			id,
			title,
			tags: []
	...
	const isLastPage = ($, spanString) => {
		return $(`span:contains("${spanString}")`).toArray().length == 0;
	};

//#endregion
//#region src/WeebCentral/helpers.ts
	...
	function getTagFromTagStore(tagId, tags) {
		...
		if (tag === void 0) throw new Error(`${tagId} Tag section not found`);
		return tag;
	}
	...
	async function getSearchTags() {
		...
		tags = await parseTags(load(Application.arrayBufferToUTF8String(buffer)));
		Application.setState(tags, "tags");
		return tags;
	}

//#endregion
//#region src/WeebCentral/forms.ts
	var SettingsForm = class extends Form {
		getSections() {
			return [Section("tags", [ButtonRow("clearTags", {
				title: "Clear Cached Search Tags",
				onSelect: Application.Selector(this, "clearTags")
			})])];
		}
		async clearTags() {
	...
	var WeebCentralAdvancedSearchForm = class extends AdvancedSearchForm {
		constructor(searchQuery, tags) {
			super();
		...
		getSeriesStatusFilter(tags) {
			const tag = getTagFromTagStore("included_status", tags);
			return [SelectRow("seriesStatus", {
				title: tag.title,
				...
				onValueChange: Application.Selector(this, "handleSeriesStatusChange")
			})];
		}
		async handleSeriesStatusChange(value) {
			this.searchMetadata.seriesStatuses = value;
		}
		getSeriesTypesFilter(tags) {
			const tag = getTagFromTagStore("included_type", tags);
			return [SelectRow("seriesType", {
				title: "Series Type",
				...
				onValueChange: Application.Selector(this, "handleSeriesTypeChange")
			})];
		}
		async handleSeriesTypeChange(value) {
			this.searchMetadata.seriesTypes = value;
		}
		getOrderFilter() {
			return [ToggleRow("order", {
				title: "Order Descending",
				subtitle: "Toggle on to sort in descending order",
				value: this.searchMetadata.orderIsDescending ?? false,
				onValueChange: Application.Selector(this, "handleOrderChange")
			})];
		}
...
//#endregion
//#region src/WeebCentral/pbconfig.ts
...
//#endregion
//#region src/WeebCentral/main.ts
	var WeebCentralExtension = class {
		constructor() {
			...
			_defineProperty(this, "requestManager", new WeebCentralInterceptor("main"));
		}
		async initialise() {
		...
		async getDiscoverSectionItems(section, metadata) {
			...
			switch (section.id) {
				case "recommended": {
					...
					items = await parseRecommendedSection(load(Application.arrayBufferToUTF8String(buffer)));
					break;
				}
				case "recent": {
					let $;
					if (page == 1) {
						const [_, buffer] = await fetchHomepage();
						$ = load(Application.arrayBufferToUTF8String(buffer));
					...
					metadata = !isLastPage($, "View More...") ? { page: page + 1 } : void 0;
					break;
				}
				case "hot": {
					const [_, buffer] = await fetchHomepage();
					items = await parseHotSection(load(Application.arrayBufferToUTF8String(buffer)));
					break;
				}
				case "genres": items = (await this.getGenres()).map((genre) => ({
					type: "genresCarouselItem",
					searchQuery: {
						title: "",
						metadata: { genres: [genre.id] }
					},
					name: genre.title,
					metadata
				}));
		...
		async getMangaDetails(mangaId) {
			...
			return await parseMangaDetails(load(Application.arrayBufferToUTF8String(buffer)), mangaId);
		}
		async getChapters(sourceManga) {
			const [_, buffer] = await fetchChaptersPage(sourceManga.mangaId);
			return parseChapters(load(Application.arrayBufferToUTF8String(buffer)), sourceManga);
		}
		async getChapterDetails(chapter) {
			const [_, buffer] = await fetchChapterDetailsPage(chapter.chapterId);
			return parseChapterDetails(load(Application.arrayBufferToUTF8String(buffer)), chapter.sourceManga.mangaId, chapter.chapterId);
		}
		...
		async getGenres() {
			...
			const genreTag = tags.find((tag) => tag.id === "included_tag");
			if (genreTag === void 0) throw new Error("Genres tag section not found");
			if (isInvalidTags(genreTag.tags)) throw new Error(`Please reset ${pbconfig_default.name} state in settings`);
		...
		async getSearchResults(query, metadata, sortingOption) {
			...
			if (query.title) queries.push(newQuery("text", query.title));
			queries.push(newQuery("included_tag", query.metadata.genres ?? []), newQuery("included_status", query.metadata.seriesStatuses ?? []), newQuery("included_type", query.metadata.seriesTypes ?? []));
			const orderIsDescending = query.metadata.orderIsDescending ?? false;
			queries.push(newQuery("order", orderIsDescending ? "Descending" : "Ascending"));
			...
			metadata = isLastPage($, "View More Results...") ? void 0 : {
				...metadata,
				offset: offset + LIMIT
			};
			return {
		...
		async getAdvancedSearchForm(searchQuery) {
			return new WeebCentralAdvancedSearchForm(searchQuery, await getSearchTags());
		}
	...
	const WeebCentral = new WeebCentralExtension();
...
//#endregion
//#region src/tests/WeebCentral.ts
	async function runTests(logger) {
		const suite = new TestSuite("WeebCentral tests", logger);
		registerDefaultTests(suite, WeebCentral, pbconfig_default);
		await suite.run();
	}
