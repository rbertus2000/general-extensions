...
//#endregion
//#region src/QToon/implementations/settings-form/forms.ts
	const LANGUAGES = [
		{
			id: "en-US",
			title: "English"
	...
	var QToonSettingsForm = class extends Form {
		getSections() {
			return [Section({
...
//#endregion
//#region src/QToon/implementations/settings-form/main.ts
	function getLanguage() {
		return Application.getState("qtoon-language") ?? "en-US";
	}
...
//#endregion
//#region src/QToon/implementations/shared/models.ts
	const DOMAIN = "https://qtoon.com";
	const DOMAIN_API = "https://api.qtoon.com";

//#endregion
//#region src/QToon/implementations/shared/utils.ts
	...
	function generateDid(length = 24) {
		let result = "";
		for (let i = 0; i < length; i++) result += DID_CHARS.charAt(Math.floor(Math.random() * 60));
		return result;
	}
	function md5(input) {
	...
	async function aesDecrypt(data, key, iv) {
		...
		const cryptoKey = await subtle.importKey("raw", keyBytes.buffer, { name: "AES-CBC" }, false, ["decrypt"]);
		const cipherBuffer = Application.base64Decode(data);
	...
	async function decryptResponse(data, ts, did) {
		const outer = md5(`${md5(`${did}${ts}`)}OQlM9JBJgLWsgffb`);
		return await aesDecrypt(data, outer.substring(0, 16), outer.substring(16, 32));
	}
	async function decryptImageUrl(url, did) {
		const outer = md5(`${md5(did)}9tv86uBwmOYs7QZ0`);
		return await aesDecrypt(url, outer.substring(0, 16), outer.substring(16, 32));
	}

//#endregion
//#region src/QToon/services/network.ts
	var QToonInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
					...
					...request.url.includes("https://api.qtoon.com") && {
						platform: "h5",
						lth: getLanguage(),
						did: requestToken
					}
	...
	async function fetchEncryptedJSON(request) {
		...
		if (envelope.code !== 0) throw new Error(`QToon API error code ${envelope.code}: ${request.url}`);
...
//#endregion
//#region src/QToon/implementations/chapter-providing/parsers.ts
	const SEASON_PATTERN = /\(?(?:S(?:eason|aison)?|T(?:emporada)?)\s*(\d+)\)?/i;
	const CHAPTER_PATTERN = /(?:chapter|chapitre|cap[ií]tulo|ch\.?|cap\.?|episode|episodio|[eé]pisode|ep\.?)\s*\d+\s*[-:–—]?\s*/gi;
	function extractSeason(title) {
		const match = title.match(SEASON_PATTERN);
		return match ? parseInt(match[1], 10) : 0;
	}
	function cleanTitle(title) {
		return title.replace(CHAPTER_PATTERN, "").replace(SEASON_PATTERN, "").replace(/^\s*[-:–—]\s*/, "").trim();
	}
	function parseQToonEpisodes(episodes, sourceManga) {
...
//#endregion
//#region src/QToon/implementations/chapter-providing/main.ts
	var ChapterProvider = class {
		async getChapters(sourceManga) {
			return parseQToonEpisodes((await fetchEncryptedJSON({
				url: new URL("https://api.qtoon.com").addPathComponent("api").addPathComponent("w").addPathComponent("comic").addPathComponent("detail").setQueryItem("csid", sourceManga.mangaId).toString(),
				method: "GET"
			})).episodes ?? [], sourceManga);
		}
		async getChapterDetails(chapter) {
			const token = (await fetchEncryptedJSON({
				url: new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("comic").addPathComponent("episode").addPathComponent("detail").setQueryItem("esid", chapter.chapterId).toString(),
				method: "GET"
			})).definitions?.[0]?.token;
			if (!token) throw new Error(`No resource token found for episode: ${chapter.chapterId}`);
			const allPages = [];
			let page = 1;
			let hasMore = true;
			while (hasMore) {
				const resourceData = await fetchEncryptedJSON({
					url: new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("resource").addPathComponent("group").addPathComponent("rslv").setQueryItem("token", token).setQueryItem("page", String(page)).toString(),
					method: "GET"
				});
				for (const resource of resourceData.resources ?? []) allPages.push({
...
//#endregion
//#region src/QToon/implementations/discover-section/parsers.ts
	function extractEndpoint(block) {
		if (block.ranking?.rsid) return {
			type: "ranking",
			id: block.ranking.rsid
		};
		if (block.album?.asid) return {
			type: "album",
			id: block.album.asid
		};
	}
	function parseCompositionBlocks(blocks) {
		return blocks.filter((block) => block.comics && block.comics.length > 0).map((block) => ({
			id: block.msid,
			title: block.title,
			type: DiscoverSectionType.simpleCarousel
		}));
	}
	function parseQToonComics(comics) {
		return comics.filter((comic) => comicId(comic)).map((comic) => ({
...
//#endregion
//#region src/QToon/implementations/discover-section/main.ts
	const HOMEPAGE_PSID = "ps_ErZj1GjyOOOaAVI1gdDj";
	function buildCompositionUrl(page) {
		return new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("navigation").addPathComponent("composition").addPathComponent("page").addPathComponent("detail").setQueryItem("platform", "h5").setQueryItem("psid", HOMEPAGE_PSID).setQueryItem("page", String(page)).toString();
	}
	function buildPaginatedUrl(type, id, page) {
		if (type === "album") return new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("album").addPathComponent("page").addPathComponent("comics").setQueryItem("page", String(page)).setQueryItem("asid", id).toString();
		return new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("ranking").addPathComponent("page").addPathComponent("comics").setQueryItem("page", String(page)).setQueryItem("rsid", id).toString();
	}
	...
	var DiscoverProvider = class {
		async getDiscoverSections() {
			...
			let hasMore = true;
			while (hasMore) {
				const data = await fetchEncryptedJSON({
					url: buildCompositionUrl(compositionPage),
					method: "GET"
				});
		...
		async getDiscoverSectionItems(section, metadata) {
			...
			if (!endpointId) {
				...
				if (!endpoint) return {
					items: parseQToonComics(block.comics ?? []),
					metadata: void 0
				};
			...
			const data = await fetchEncryptedJSON({
				url: buildPaginatedUrl(endpointType, endpointId, page),
				method: "GET"
			});
			return {
				items: parseQToonComics(data.comics ?? []),
				metadata: data.more === 1 ? {
					page: page + 1,
					endpointType,
					endpointId
				} : void 0
...
//#endregion
//#region src/QToon/implementations/manga/parsers.ts
	function parseQToonMangaDetails(comic, mangaId) {
		const tags = [...comic.tags.map((t) => ({
		...
		switch (comic.serialStatus2) {
			...
			case 103:
				status = "Completed";
				break;
			default: status = void 0;
		}
		const synopsis = comic.updateMemo ? `${comic.introduction}\n\nUpdates: ${comic.updateMemo}` : comic.introduction;
		return {
			mangaId,
			mangaInfo: {
...
//#endregion
//#region src/QToon/implementations/manga/main.ts
	var MangaProvider = class {
		async getMangaDetails(mangaId) {
			return parseQToonMangaDetails((await fetchEncryptedJSON({
				url: new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("comic").addPathComponent("detail").setQueryItem("csid", mangaId).toString(),
				method: "GET"
			})).comic, mangaId);
		}
	};
...
//#endregion
//#region src/QToon/implementations/search-results/parsers.ts
	function readDropdownFilter(filters, filterId, fallback) {
		const entry = filters.find((f) => f.id === filterId);
		if (!entry) return fallback;
		const val = entry.value;
		return typeof val === "string" && val.trim() ? val.trim() : fallback;
	}
	function parseQToonSearchResults(comics) {
		return comics.filter((comic) => comicId(comic)).map((comic) => ({
			mangaId: comicId(comic),
			title: comic.title ?? "",
			subtitle: comic.author ?? "",
...
//#endregion
//#region src/QToon/implementations/search-results/main.ts
	var SearchProvider = class {
		...
		async getSearchResults(query, metadata, sortingOption) {
			...
			const base = new URL(DOMAIN_API).addPathComponent("api").addPathComponent("w").addPathComponent("search").addPathComponent("comic");
			const data = await fetchEncryptedJSON({
				url: title ? base.addPathComponent("search").setQueryItem("title", title).setQueryItem("page", String(page)).toString() : base.addPathComponent("gallery").setQueryItem("area", "-1").setQueryItem("tag", tag).setQueryItem("gender", "-1").setQueryItem("serialStatus", status).setQueryItem("sortType", sortType).setQueryItem("page", String(page)).toString(),
				method: "GET"
			});
			return {
				items: parseQToonSearchResults(data.comics ?? []),
				metadata: data.more === 1 ? { page: page + 1 } : void 0
			};
		}
	};

//#endregion
//#region src/QToon/main.ts
	const requestToken = generateDid();
	var QToonExtension = class {
		constructor() {
			...
			_defineProperty(this, "qtoonInterceptor", new QToonInterceptor("qtoon-interceptor"));
		}
		async initialise() {
	...
	const QToon = new QToonExtension();

//#endregion
//#region src/QToon/pbconfig.ts
	var pbconfig_default = {
		...
		icon: "icon.png",
		language: "multi",
		contentRating: ContentRating$1.MATURE,
		capabilities: [
...
//#endregion
//#region src/tests/QToon.ts
	async function runTests(logger) {
		const suite = new TestSuite("QToon tests", logger);
		registerDefaultTests(suite, QToon, pbconfig_default);
		await suite.run();
	}
