...
//#endregion
//#region src/MangaTaro/implementations/shared/models.ts
	const DOMAIN = "https://mangataro.org";

//#endregion
//#region src/MangaTaro/services/network.ts
	var MangaTaroInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#endregion
//#region src/MangaTaro/implementations/shared/utils.ts
	...
	function generateToken() {
		...
		const secret = "mng_ch_" + (/* @__PURE__ */ new Date()).toISOString().slice(0, 13).replace(/[-T:]/g, "");
		const array = new TextEncoder().encode(timestamp.toString() + secret);
		return {
			token: Application.crypto_md5Hash(array.buffer).substring(0, 16),
			timestamp
		};
	}
	function isNovel(type) {
	...
	function parseMangaId(mangaId) {
		...
		return {
			slug: mangaId.substring(0, idx),
			numericId: mangaId.substring(idx + 1)
		};
	}
	function formatMangaId(slug, numericId) {
	...
	function parseRelativeDate(str) {
		...
		if (!match) return now;
		const amount = match[1] !== void 0 ? parseInt(match[1], 10) : 1;
		const unit = match[2];
		const ms = now.getTime();
		switch (unit) {
			case "second": return /* @__PURE__ */ new Date(ms - amount * 1e3);
			case "minute": return /* @__PURE__ */ new Date(ms - amount * 6e4);
			case "hour": return /* @__PURE__ */ new Date(ms - amount * 36e5);
			case "day": return /* @__PURE__ */ new Date(ms - amount * 864e5);
			case "week": return /* @__PURE__ */ new Date(ms - amount * 7 * 864e5);
			case "month": return /* @__PURE__ */ new Date(ms - amount * 30 * 864e5);
			case "year": return /* @__PURE__ */ new Date(ms - amount * 365 * 864e5);
			default: return now;
		}
	}

//#endregion
//#region src/MangaTaro/implementations/chapter-providing/parsers.ts
	function parseChapterList(chapters, sourceManga) {
...
//#endregion
//#region src/MangaTaro/implementations/chapter-providing/main.ts
	var ChapterProvider = class {
		async getChapters(sourceManga) {
			...
			if (!numericId || !/^\d+$/.test(numericId)) {
				const resolved = extractNumericId(await fetchText({
					url: new URL(DOMAIN).addPathComponent("manga").addPathComponent(slug).toString(),
					method: "GET"
				}));
			...
			while (hasMore) {
				...
				const data = await fetchJSON({
					url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("manga-chapters").setQueryItem("manga_id", numericId).setQueryItem("offset", String(offset)).setQueryItem("limit", String(LIMIT)).setQueryItem("order", "DESC").setQueryItem("_t", token).setQueryItem("_ts", timestamp.toString()).toString(),
					method: "GET"
				});
		...
		async getChapterDetails(chapter) {
			...
			const data = await fetchJSON({
				url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("chapter-content").setQueryItem("chapter_id", chapterId).toString(),
				method: "GET"
			});
			return {
				id: chapterId,
...
//#endregion
//#region src/MangaTaro/implementations/discover-section/parsers.ts
	function parsePopularChapters(chapters) {
		return chapters.filter((ch) => !isNovel(ch.manga_type)).map((ch) => ({
	...
	function parseStatusManga(items) {
		return items.filter((item) => !isNovel(item.manga_type)).map((item) => ({
	...
	function parseFollowedManga(items) {
		return items.filter((item) => !isNovel(item.manga_type)).map((item) => ({
			type: "simpleCarouselItem",
			mangaId: formatMangaId(item.slug, item.manga_id),
			imageUrl: item.cover,
	...
	function parsePopularManga(items) {
		return items.filter((item) => !isNovel(item.manga_type)).map((item) => ({
			type: "simpleCarouselItem",
...
//#endregion
//#region src/MangaTaro/implementations/discover-section/main.ts
	const SECTIONS = [
		{
			id: "popular-chapters",
		...
		async getDiscoverSectionItems(section, _metadata) {
			...
			switch (section.id) {
				case "popular-chapters":
					items = parsePopularChapters((await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("popular-chapters").setQueryItem("period", "today").setQueryItem("limit", "15").toString(),
						method: "GET"
					})).chapters);
					break;
				case "manga-status":
					items = parseStatusManga((await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("manga-status-slider").setQueryItem("status", "completed").setQueryItem("limit", "15").toString(),
						method: "GET"
					})).manga);
					break;
				case "most-followed-new":
					items = parseFollowedManga((await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("most-followed-new-manga").setQueryItem("period", "7d").setQueryItem("limit", "15").toString(),
						method: "GET"
					})).manga);
					break;
				case "most-followed":
					items = parseFollowedManga((await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("auth").addPathComponent("most-followed-manga").setQueryItem("period", "month").setQueryItem("limit", "15").toString(),
						method: "GET"
					})).manga);
					break;
				case "popular-manga":
					items = parsePopularManga(await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("wp-json").addPathComponent("manga").addPathComponent("v1").addPathComponent("popular").setQueryItem("period", "week").setQueryItem("number", "15").toString(),
						method: "GET"
					}));
					break;
				case "high-score":
					items = parsePopularManga(await fetchJSON({
						url: new URL(DOMAIN).addPathComponent("wp-json").addPathComponent("manga").addPathComponent("v1").addPathComponent("highscore").setQueryItem("type", "all").setQueryItem("number", "15").toString(),
						method: "GET"
					}));
					break;
				default: items = [];
...
//#endregion
//#region src/MangaTaro/implementations/manga/parsers.ts
	function parseMangaDetails(html, mangaId) {
		const scriptMatch = html.match(/<script[^>]*>\s*(\{[\s\S]*?"@type"\s*:\s*"ComicSeries"[\s\S]*?\})\s*<\/script>/);
		if (!scriptMatch) throw new Error("Could not find Schema.org ComicSeries data in page");
		...
		const primaryTitle = schema.name.replace(/\s*\|.*$/, "").replace(/\s+(Manhwa|Manga|Manhua|Webtoon)$/i, "").trim();
		const synopsisMatch = html.match(/class="[^"]*text-justify[^"]*"[^>]*>\s*<p>([\s\S]*?)<\/p>/);
		const synopsis = (synopsisMatch ? synopsisMatch[1].replace(/<[^>]+>/g, "").trim() : schema.description ?? "").replace(/&#(\d+);/g, (_, code) => String.fromCharCode(parseInt(code, 10)));
		const tagGroups = schema.genre ? [{
			id: "tags",
			title: "Tags",
			tags: [{
				id: schema.genre.toLowerCase(),
				title: schema.genre
			}]
		}] : [];
		return {
...
//#endregion
//#region src/MangaTaro/implementations/manga/main.ts
...
//#endregion
//#region src/MangaTaro/implementations/search-results/parsers.ts
	const TYPES = [
		"Manga",
		"Manhwa",
		"Manhua"
	...
	const YEARS = [];
	for (let y = CURRENT_YEAR; y >= 1946; y--) YEARS.push(String(y));
	const SORT_OPTIONS = [
		{
			id: "post_desc",
	...
	async function fetchAllTags() {
		return fetchJSON({
			url: new URL(DOMAIN).addPathComponent("wp-json").addPathComponent("wp").addPathComponent("v2").addPathComponent("tags").setQueryItem("per_page", "100").toString(),
			method: "GET"
		});
	}
	async function buildSearchFilters() {
		const genreOptions = (await fetchAllTags()).filter((t) => t.count > 0).sort((a, b) => a.name.localeCompare(b.name)).map((t) => ({
			id: String(t.id),
			value: t.name
		}));
		return [
	...
	function readMultiselectFilter(filters, filterId) {
		const entry = filters.find((f) => f.id === filterId);
		if (!entry) return [];
		const val = entry.value;
		if (typeof val === "string") return [];
		return Object.entries(val).filter(([, state]) => state === "included").map(([id]) => id);
	}
...
//#endregion
//#region src/MangaTaro/implementations/search-results/main.ts
	...
	var SearchProvider = class {
		...
		async getSearchResults(query, metadata, sortingOption) {
			...
			const url = new URL(DOMAIN).addPathComponent("wp-json").addPathComponent("manga").addPathComponent("v1").addPathComponent("load").toString();
			const body = {
				page,
				search: searchTerm,
			...
			return {
				items: data.filter((item) => !isNovel(item.type)).map((item) => ({
				...
				metadata: data.length > 0 ? { page: page + 1 } : void 0
			};
		}
	};

//#endregion
//#region src/MangaTaro/main.ts
	var MangaTaroExtension = class {
		constructor() {
			...
			_defineProperty(this, "mangaTaroInterceptor", new MangaTaroInterceptor("mangataro-interceptor"));
		}
		async initialise() {
	...
	applyMixins(MangaTaroExtension, [
		SearchProvider,
		MangaProvider,
		ChapterProvider,
		DiscoverProvider
	]);
	const MangaTaro = new MangaTaroExtension();

//#endregion
//#region src/MangaTaro/pbconfig.ts
...
//#endregion
//#region src/tests/MangaTaro.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaTaro tests", logger);
		registerDefaultTests(suite, MangaTaro, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=MangaTaro.js.map