...
//#endregion
//#region src/MangaKatana/genreOptions.ts
	const genreOptions = [
		{
			id: "1",
			value: "4 koma"
...
//#endregion
//#region src/MangaKatana/genres.ts
	const genres = [
		{
			id: "1",
			name: "4 koma"
...
//#endregion
//#region src/MangaKatana/MangaKatanaParser.ts
	const parseTags = ($) => {
		const arrayTags = [];
	...
	const parseSearch = ($) => {
		...
		} else for (const manga of $("div.item", "#book_list").toArray()) {
	...
	const isLastPage = ($) => {
		let isLast = true;
		if (Boolean($("a.next.page-numbers", "ul.uk-pagination").text())) isLast = false;
		return isLast;
	};

//#endregion
//#region src/MangaKatana/models.ts
	const DOMAIN = "https://mangakatana.com";

//#endregion
//#region src/MangaKatana/network.ts
	var MangaKatanaInterceptor = class extends PaperbackInterceptor {
		async interceptRequest(request) {
			return {
				...request,
				headers: {
...
//#endregion
//#region src/MangaKatana/main.ts
	var MangaKatanaExtension = class {
		constructor() {
			...
			_defineProperty(this, "mangaKatanaInterceptor", new MangaKatanaInterceptor("main"));
		...
		async getDiscoverSectionItems(section, metadata) {
			switch (section.id) {
				case "hot-updates": return this.getHotUpdatesSectionItems();
				case "latest-updates": return this.getLatestUpdatesSectionItems(section, metadata);
				case "new-manga": return this.getNewMangaSectionItems(section, metadata);
				case "genres": return this.getGenresSectionItems();
		...
		async getHotUpdatesSectionItems() {
			...
			$("#hot_update .item").each((_, element) => {
				...
				let mangaId = (titleLink.attr("href") || "").split("/").pop() || "";
				...
				if (mangaId && title && image) items.push(createDiscoverSectionItem({
					id: mangaId,
					image,
					title,
					subtitle: chapter,
		...
		async getLatestUpdatesSectionItems(section, metadata) {
			...
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("page").addPathComponent(page.toString()).toString(),
				method: "GET"
			};
			const $ = await this.fetchCheerio(request);
			const items = [];
			$("#book_list .item").each((_, element) => {
				...
				mangaId = decodeURIComponent(mangaId).replace(/[^\w@.]/g, "_").trim();
				const image = unit.find(".wrap_img img").attr("src") || "";
				const latestChapter = unit.find(".chapters .chapter a").first().text().trim();
			...
			if (nextPageHref) {
				...
				if (pageMatch) nextPage = parseInt(pageMatch[1], 10);
		...
		async getNewMangaSectionItems(section, metadata) {
			...
			const collectedIds = metadata?.collectedIds ?? [];
			const request = {
				url: new URL$1(DOMAIN).addPathComponent("new-manga").addPathComponent("page").addPathComponent(page.toString()).toString(),
				method: "GET"
			};
			const $ = await this.fetchCheerio(request);
			const items = [];
		...
		async getSearchTags() {
			...
			return parseTags(await this.fetchCheerio(request));
		}
		async getSearchResults(query, metadata) {
			const page = metadata?.page ?? 1;
			if (query.title && query.title.length === 1) return {
				items: [],
				metadata: void 0
			};
			let request;
			if (query.title) request = {
				url: new URL$1(DOMAIN).addPathComponent("page").addPathComponent(page.toString()).setQueryItem("search", query.title).setQueryItem("search_by", "book_name").toString(),
				method: "GET"
			};
			else {
				const genreValue = (query.metadata?.find((f) => f.id === "genres"))?.value;
				const includeValue = Object.entries(genreValue || {}).filter(([, value]) => value === "included").map(([id]) => id).map((id) => {
					const genreOption = genreOptions.find((option) => option.id === id);
					return genreOption ? genreOption.value.toLowerCase().replace(/ /g, "_") : "";
				}).filter(Boolean).join("_");
				request = {
					url: new URL$1(DOMAIN).addPathComponent("genres").addPathComponent("page").addPathComponent(page.toString()).setQueryItem("filter", "1").setQueryItem("include", includeValue).setQueryItem("include_mode", "and").setQueryItem("bookmark_opts", "off").setQueryItem("chapters", "1").toString(),
					method: "GET"
				};
			}
			try {
				...
				console.log(`\n\n It failed from here \n\n`);
				const nextPageMeta = !isLastPage($) ? { page: page + 1 } : void 0;
		...
		async getChapters(sourceManga) {
			...
			$(".chapters table.uk-table tbody tr").each((_, element) => {
				...
				const chapterMatch = chapterLink.text().trim().match(/Chapter\s+([\d.]+)(?:\s*-\s*(.*))?/i);
				const chapterNumber = chapterMatch ? parseFloat(chapterMatch[1]) : 0;
				const formattedTitle = chapterMatch?.[2]?.trim() || "";
				const [month, day, year] = row.find(".update_time").text().trim().split("-");
		...
		async getChapterDetails(chapter) {
			...
			try {
				...
				if (response.status !== 200) throw new Error(`Failed to fetch chapter data: HTTP ${response.status}`);
				const htmlStr = Application.arrayBufferToUTF8String(data);
				const $ = load(htmlStr);
				let pages = [];
				const scripts = $("script").toArray().filter((script) => $(script).text().includes("var ytaw") || $(script).text().includes("var thzq")).map((script) => $(script).text()).join("");
				const ytawMatch = scripts.match(/var ytaw\s*=\s*\[([^\]]+)\]/);
				const thzqMatch = scripts.match(/var thzq\s*=\s*\[([^\]]+)\]/);
				const parseUrls = (matchStr) => {
					if (!matchStr) return [];
					return matchStr[1].split(",").map((url) => url.trim().replace(/['"]/g, "")).filter((url) => url && !url.includes("about:blank")).map((url) => url.startsWith("http") ? url : `${DOMAIN}${url}`);
				};
				pages = [...parseUrls(ytawMatch), ...parseUrls(thzqMatch)];
				if (pages.length === 0) $("#imgs .wrap_img img").each((_, img) => {
				...
				if (pages.length === 0) throw new Error("No valid image URLs found");
				return {
		...
		async getMangaDetails(mangaId) {
			...
			if (genres.some((genre) => adultGenres.includes(genre))) contentRating = ContentRating$1.ADULT;
			else if (genres.some((genre) => matureGenres.includes(genre))) contentRating = ContentRating$1.MATURE;
			return {
				mangaId,
				mangaInfo: {
					primaryTitle: title,
	...
	const MangaKatana = new MangaKatanaExtension();

//#endregion
//#region src/MangaKatana/pbconfig.ts
...
//#endregion
//#region src/tests/MangaKatana.ts
	async function runTests(logger) {
		const suite = new TestSuite("MangaKatana tests", logger);
		registerDefaultTests(suite, MangaKatana, pbconfig_default);
		await suite.run();
	}
...
//# sourceMappingURL=MangaKatana.js.map